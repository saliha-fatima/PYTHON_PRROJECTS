...
def display_report():
    ...
    report_window.title("Hotel Report")

    # Display title
    tk.Label(report_window, text="Hotel Data Report", font=("Arial", 16, "bold")).grid(row=0, column=0, columnspan=4, pady=10)

    # Room data
    tk.Label(report_window, text="Room Details", font=("Arial", 14, "bold")).grid(row=1, column=0, columnspan=4, pady=5)
    tk.Label(report_window, text="Room Number", font=("Arial", 12, "bold")).grid(row=2, column=0, padx=10)
    tk.Label(report_window, text="Room Type", font=("Arial", 12, "bold")).grid(row=2, column=1, padx=10)
    tk.Label(report_window, text="Status", font=("Arial", 12, "bold")).grid(row=2, column=2, padx=10)
    tk.Label(report_window, text="Rate", font=("Arial", 12, "bold")).grid(row=2, column=3, padx=10)

    # Loop through the rooms dictionary and display room details
    for idx, (room_num, room) in enumerate(rooms.items(), start=3):
        tk.Label(report_window, text=room_num, font=("Arial", 12)).grid(row=idx, column=0, padx=10)
        tk.Label(report_window, text=room["type"], font=("Arial", 12)).grid(row=idx, column=1, padx=10)
        tk.Label(report_window, text=room["status"], font=("Arial", 12)).grid(row=idx, column=2, padx=10)
        tk.Label(report_window, text=f"${room.get('rate', 0):.2f}", font=("Arial", 12)).grid(row=idx, column=3, padx=10)

    # Guest data
    guest_start_row = len(rooms) + 5
    tk.Label(report_window, text="Guest Details", font=("Arial", 14, "bold")).grid(row=guest_start_row, column=0, columnspan=4, pady=5)
    tk.Label(report_window, text="Token", font=("Arial", 12, "bold")).grid(row=guest_start_row + 1, column=0, padx=10)
    tk.Label(report_window, text="Guest Name", font=("Arial", 12, "bold")).grid(row=guest_start_row + 1, column=1, padx=10)
    tk.Label(report_window, text="Room", font=("Arial", 12, "bold")).grid(row=guest_start_row + 1, column=2, padx=10)
    tk.Label(report_window, text="Days of Stay", font=("Arial", 12, "bold")).grid(row=guest_start_row + 1, column=3, padx=10)

    # Loop through the guests dictionary and display guest details
    for idx, (token, guest) in enumerate(guests.items(), start=guest_start_row + 2):
        tk.Label(report_window, text=token, font=("Arial", 12)).grid(row=idx, column=0, padx=10)
        tk.Label(report_window, text=guest["name"], font=("Arial", 12)).grid(row=idx, column=1, padx=10)
        tk.Label(report_window, text=guest["room"], font=("Arial", 12)).grid(row=idx, column=2, padx=10)
        tk.Label(report_window, text=guest["num_days"], font=("Arial", 12)).grid(row=idx, column=3, padx=10)

    # Close button
    tk.Button(report_window, text="Close", command=report_window.destroy, font=("Arial", 12)).grid(row=idx + 1, column=0, columnspan=4, pady=20)
...
def save_data_to_file():
    data = {
        ...
        "guest_token_counter": guest_token_counter,
    }
    try:
        with open("hotel_data.json", "w") as file:
            json.dump(data, file, indent=4)
        messagebox.showinfo("Success", "Data saved successfully to 'hotel_data.json'.")
...
def show_main_menu():
    ...
    button_list = [
        ...
        ("Checkout", checkout),
        ("File Complaint", file_complaint),
        ("Display Report", display_report),
        ("Save Data", save_data_to_file),  # New button for saving data
        ("Exit", exit_application)  # New button for exiting the application
    ]
...
def show_login_screen():
    ...
    tk.Button(root, text="Exit", command=exit_application, font=("Arial", 12)).grid(row=2, column=0, padx=20, pady=20)  # Exit button on login screen
